...
import static io.angularpay.supply.models.UserNotificationType.BARGAIN_REJECTED;
...
public class RejectBargainCommand extends AbstractCommand<RejectBargainCommandRequest, GenericCommandResponse>
    ...
    public String convertToUserNotificationsMessage(UserNotificationBuilderParameters<GenericCommandResponse, SupplyRequest> parameters) throws JsonProcessingException {
        ...
        if (optional.isPresent() && optional.get().equalsIgnoreCase(parameters.getUserReference())) {
            summary = "the bargain you made on a Supply post, was rejected";
        } else {
            summary = "someone's bargain on a Supply post that you commented on, was rejected";
        }
